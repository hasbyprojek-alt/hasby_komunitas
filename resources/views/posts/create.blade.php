...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-extrabold text-slate-800">✍️ Buat Diskusi Baru</h1>
        <p class="text-slate-500 mt-1 text-sm">Bagikan ide atau pengalamanmu kepada komunitas.</p>
    </div>
        ...
        <form action="{{ route('posts.store') }}" method="POST">
            ...
            <div class="mb-7">
                ...
                <textarea id="content" name="content" rows="8"
                          placeholder="Ceritakan sesuatu yang bermanfaat untuk komunitas..."
                          class="w-full border {{ $errors->has('content') ? 'border-red-400 bg-red-50' : 'border-slate-200' }}
                                 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none">{{ old('content') }}</textarea>
                @error('content')
                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
